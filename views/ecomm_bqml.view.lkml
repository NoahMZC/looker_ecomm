    ...
    sql: (SELECT * except(user_id1) FROM
              (SELECT user_id
                ...
                FROM (
                  ...
                  FROM `mzcdsc-team-200716.ecomm.order_items` as oi
                  JOIN `mzcdsc-team-200716.ecomm.inventory_items` as inv ON inv.id = oi.inventory_item_id
...
view: ecomm_predict {
  derived_table: {
    sql: SELECT
      ...
      FROM
        ML.PREDICT(MODEL `mzcdsc-team-200716.ecomm.kmeans_model5`,
        (SELECT * FROM
